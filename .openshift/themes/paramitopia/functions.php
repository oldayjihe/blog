...
echo '<link rel="shortcut icon" href="'.get_home_url().'/favicon.ico" type="image/x-icon" />';
}
add_action( 'admin_head', 'adminfavicon' );
add_action( 'wp_head', 'adminfavicon' );
...
function paramitopia_options_items() {
	$items = array (
		...
		array(
			'id' => 'googleplus_url',
			'name' => __('googleplus URL', 'paramitopia'),
			'desc' => __('Put your full googleplus address here.(with https:// , leave it blank for no display none.)', 'paramitopia')
		),
		array(
			'id' => 'excerpt_check',
...
// Copyright Output
function output_copyright($content) {
	if ( is_single() or is_feed() or is_page() ) {
		$content .= "<blockquote>";
		$content .= '<div class="copyright">&mdash;&mdash;</br>';
		$content .= '本作品由&nbsp;<a href="'.get_home_url().'">风的守候</a>&nbsp;创作.&nbsp;转载请注明出处:&nbsp;<a rel="bookmark" title="'.get_the_title().'" href="'.get_permalink().'">'.get_the_title().'</a>&nbsp;.</br>';
		$content .= '本博客采用<a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/3.0/">创作共用版权协议</a>,&nbsp;要求&ldquo;署名-非商业用途-禁止演绎&rdquo;.&nbsp;转载本博客内容必须也遵循该协议.</br>';
		$content .= 'This blog is licensed under a Creative Commons Attribution-NonCommercial-NoDerivs 3.0 <a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/3.0/">License</a>.</div>';
		$content .= "</blockquote>";
	}
	return $content;
}
